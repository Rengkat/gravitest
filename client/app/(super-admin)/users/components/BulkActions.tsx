...
export function BulkActionsBar({
  count, onActivate, onSuspend, onDeactivate, onDelete, onExport, onClear,
}: BulkProps) {
  return (
    <div className="mb-4 p-4 rounded-2xl bg-green-800 text-white flex items-center justify-between flex-wrap gap-3">
      <div className="flex items-center gap-3">
        <Check size={18} />
        <span className="text-[14px] font-semibold">{count} user{count > 1 ? "s" : ""} selected</span>
      </div>
...
function BulkBtn({ Icon, label, onClick, danger = false }: { Icon: any; label: string; onClick: () => void; danger?: boolean }) {
  return (
...
// ─── ADD USER MODAL ──────────────────────────────────────────
interface AddUserProps { onClose: () => void; onAdd: (user: any) => void; }
...
export function AddUserModal({ onClose, onAdd }: AddUserProps) {
  ...
  const handleSubmit = () => {
    ...
    onAdd({
      ...
      studentProfile: form.role === "student" ? {
        ...
        xpPoints: 0, streak: 0, averageScore: 0, sessionsCompleted: 0, totalStudyHours: 0,
        schoolName: form.schoolName || undefined,
      } : undefined,
      tutorProfile: form.role === "tutor" ? {
        subjects: form.subjects ? form.subjects.split(",").map((s) => s.trim()) : [],
        qualifications: [], yearsOfExperience: 0, rating: 0, ratingCount: 0,
        totalSessionsConducted: 0, totalStudentsTaught: 0,
        isVerified: false, availabilityStatus: "available",
      } : undefined,
      schoolAdminProfile: form.role === "school_admin" ? {
        schoolId: `sch_${Date.now()}`,
        schoolName: form.schoolName,
        schoolType: "private", adminRole: "admin",
        managedClasses: [], managedStudentCount: 0, managedTeacherCount: 0,
  ...
  const inputCls = "w-full px-3 py-2.5 rounded-xl border border-gray-200 text-[13px] focus:outline-none focus:ring-2 focus:ring-green-500/30 focus:border-green-400 transition-all";

  const showSchool  = form.role === "student" && form.accountType === "school_based" || form.role === "school_admin";
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={form.firstName} onChange={(e) => set("firstName", e.target.value)}
                placeholder="e.g., Adebayo" className={inputCls} />
            </div>
            <div>
                ...
                return (
                  <button key={role} type="button" onClick={() => set("role", role)}
                    className={`flex items-center gap-2 p-3 rounded-xl border-2 transition-all text-left ${
                      form.role === role ? "border-green-800 bg-green-50" : "border-gray-200 hover:border-green-800/30"
                    }`}
                  >
                ...
                {(["individual","school_based"] as AccountType[]).map((t) => (
                  <button key={t} type="button" onClick={() => set("accountType", t)}
                    className={`p-3 rounded-xl border-2 text-[12px] font-semibold transition-all capitalize ${
                      form.accountType === t ? "border-green-800 bg-green-50 text-green-900" : "border-gray-200 text-text-muted"
                    }`}
                  >
                    {t.replace("_", " ")}
                  </button>
          ...
          {showSchool && (
            <div>
              ...
              <input type="text" value={form.schoolName} onChange={(e) => set("schoolName", e.target.value)}
                placeholder="e.g., Lagos Preparatory School" className={inputCls} />
            </div>
          )}
          ...
          {showSubjects && (
            <div>
              ...
              <input type="text" value={form.subjects} onChange={(e) => set("subjects", e.target.value)}
                placeholder="e.g., Mathematics, Physics, Chemistry" className={inputCls} />
            </div>
          )}

          <div>
            <Label>Subscription Plan</Label>
            <select value={form.subscriptionTier} onChange={(e) => set("subscriptionTier", e.target.value as SubscriptionTier)} className={inputCls}>
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={onClose} className="flex-1 py-3 rounded-xl border-2 border-gray-200 text-[14px] font-semibold text-text-muted hover:bg-gray-50 transition-all">Cancel</button>
...
function Label({ children }: { children: React.ReactNode }) {
  return <label className="block text-[12px] font-semibold text-green-900 mb-1.5">{children}</label>;
}
