...
export function Avatar({
  ...
  if (user.avatar) {
    return (
      <img src={user.avatar} alt="" className={`${dims} rounded-full object-cover shrink-0`} />
    );
  }
  return (
    <div
      className={`${dims} rounded-full bg-green-800 text-white font-bold flex items-center justify-center shrink-0`}>
      {user.firstName[0]}
      {user.lastName[0]}
    </div>
  );
