    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
        ...
        <MiniStatCard
          icon={GraduationCap}
          label="Students"
          value={stats.byRole.students.toLocaleString()}
          color="#0284c7"
        />
        <MiniStatCard
          icon={BookOpen}
          label="Tutors"
          value={stats.byRole.tutors.toLocaleString()}
          color="#8b5cf6"
        />
        <MiniStatCard
          icon={School}
          label="School Admins"
          value={stats.byRole.schoolAdmins.toLocaleString()}
          color="#f59e0b"
        />
        <MiniStatCard
          icon={UserCheck}
        ...
        <MiniStatCard
          icon={Star}
          label="Tutor Avg Rating"
          value={`${stats.tutorAvgRating} ★`}
          color="#f59e0b"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="p-6">
            ...
            {[
              {
                key: "student",
                label: "Students",
                count: stats.byRole.students,
                cfg: ROLE_CONFIG.student,
              },
              { key: "tutor", label: "Tutors", count: stats.byRole.tutors, cfg: ROLE_CONFIG.tutor },
              {
                key: "school_admin",
                label: "School Admins",
                count: stats.byRole.schoolAdmins,
                cfg: ROLE_CONFIG.school_admin,
              },
              {
                key: "super_admin",
                label: "Super Admins",
                count: stats.byRole.superAdmins,
                cfg: ROLE_CONFIG.super_admin,
              },
          ...
          {/* Account type split */}
          <div
            className="mt-6 pt-4 border-t grid grid-cols-2 gap-3"
            style={{ borderColor: "rgba(30,80,50,0.08)" }}>
            <div className="p-3 rounded-xl bg-cream/40 text-center">
              <div className="text-xl font-bold text-green-900">
                {stats.byAccountType.individual.toLocaleString()}
              </div>
              <div className="text-[11px] text-text-muted">Individual</div>
            </div>
            <div className="p-3 rounded-xl bg-cream/40 text-center">
              <div className="text-xl font-bold text-green-900">
                {stats.byAccountType.schoolBased.toLocaleString()}
              </div>
        ...
        <Card className="p-6">
          ...
          <div className="grid grid-cols-3 gap-2">
            {(
              Object.entries(SUBSCRIPTION_CONFIG) as [
                string,
                (typeof SUBSCRIPTION_CONFIG)[keyof typeof SUBSCRIPTION_CONFIG],
              ][]
            )
              .filter(([k]) => k !== "enterprise")
              .map(([tier, cfg]) => {
                const Icon = cfg.icon;
