...
export function generateMockUsers(count: number): User[] {
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    const accountType = role === "student" && Math.random() > 0.4 ? "school_based" : "individual";

    const status: UserStatus = pick(statuses);
    const tier: SubscriptionTier = role === "super_admin" ? "enterprise" : pick(tiers);

    const joinDate = new Date(2022 + rand(0, 2), rand(0, 11), rand(1, 28))
      .toISOString()
      .split("T")[0];

    const lastActive = new Date(2024, rand(0, 3), rand(1, 28)).toISOString().split("T")[0];
    ...
    return {
      ...
      studentProfile:
        role === "student" ? makeStudentProfile(accountType === "school_based") : undefined,
      ...
      tags: pickMany(
        ["nigeria", "jamb", "waec", "active_learner", "high_performer", "at_risk"],
        rand(0, 3),
      ),
    };
  });
...
export function generateMockStats(users: User[]) {
  ...
  return {
    ...
    averageSessionScore: parseFloat(
      (
        students.reduce((s, u) => s + (u.studentProfile?.averageScore ?? 0), 0) /
        (students.length || 1)
      ).toFixed(1),
    ),
    ...
    tutorAvgRating: parseFloat(
      (
        tutors.reduce((s, u) => s + (u.tutorProfile?.rating ?? 0), 0) / (tutors.length || 1)
      ).toFixed(1),
    ),
    ...
    topSubjects: SUBJECTS.slice(0, 8)
      .map((name) => ({
        name,
        students: rand(200, 2000),
      }))
