...
const DEFAULT_LIMITS: RateLimitConfig[] = [
  { tier: "free", requestsPerDay: 20, requestsPerHour: 5, tokensPerMinute: 1000, concurrentSessions: 1 },
  { tier: "basic", requestsPerDay: 100, requestsPerHour: 20, tokensPerMinute: 5000, concurrentSessions: 2 },
  { tier: "pro", requestsPerDay: 500, requestsPerHour: 100, tokensPerMinute: 20000, concurrentSessions: 5 },
  { tier: "enterprise", requestsPerDay: 5000, requestsPerHour: 1000, tokensPerMinute: 100000, concurrentSessions: 20 },
];
...
export function RateLimitControls({ onUpdate }: Props) {
  ...
  const handleUpdate = (tier: string, field: keyof RateLimitConfig, value: number) => {
    setLimits(prev =>
      prev.map(l => l.tier === tier ? { ...l, [field]: value } : l)
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-4 rounded-xl bg-amber-50 border border-amber-200 flex items-start gap-3">
        ...
        <div>
          ...
          <div className="text-[12px] text-amber-700 mt-1">
            Lower limits for free tier help manage costs. Adjust limits carefully to balance user experience and budget.
          </div>
        </div>
      </div>
      ...
      {/* Rate Limit Table */}
      <div className="bg-white rounded-2xl border overflow-hidden" style={{ borderColor: "rgba(30,80,50,0.08)" }}>
        <div className="px-6 py-4 bg-cream/30 border-b" style={{ borderColor: "rgba(30,80,50,0.08)" }}>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b" style={{ borderColor: "rgba(30,80,50,0.08)" }}>
                <th className="text-left px-6 py-3 text-[11px] font-semibold text-text-muted uppercase">Tier</th>
                <th className="text-right px-6 py-3 text-[11px] font-semibold text-text-muted uppercase">Requests/Day</th>
                <th className="text-right px-6 py-3 text-[11px] font-semibold text-text-muted uppercase">Requests/Hour</th>
                <th className="text-right px-6 py-3 text-[11px] font-semibold text-text-muted uppercase">Tokens/Min</th>
                <th className="text-right px-6 py-3 text-[11px] font-semibold text-text-muted uppercase">Concurrent Sessions</th>
              </table>
            </thead>
                ...
                <tr key={limit.tier} className="hover:bg-cream/20">
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded-lg text-[11px] font-semibold ${
                      limit.tier === "free" ? "bg-gray-100 text-gray-600" :
                      limit.tier === "basic" ? "bg-blue-100 text-blue-600" :
                      limit.tier === "pro" ? "bg-green-100 text-green-600" : "bg-purple-100 text-purple-600"
                    }`}>
                  ...
                  <td className="px-6 py-4">
                    <input
                      ...
                      onChange={(e) => handleUpdate(limit.tier, "requestsPerDay", parseInt(e.target.value))}
                  ...
                  <td className="px-6 py-4">
                    <input
                      ...
                      onChange={(e) => handleUpdate(limit.tier, "requestsPerHour", parseInt(e.target.value))}
                      className="w-32 px-3 py-1.5 rounded-lg border border-gray-200 text-right text-[13px] focus:outline-none focus:ring-2 focus:ring-green-500/30"
                    />
                  </td>
                  <td className="px-6 py-4">
                    <input
                      ...
                      onChange={(e) => handleUpdate(limit.tier, "tokensPerMinute", parseInt(e.target.value))}
                      className="w-32 px-3 py-1.5 rounded-lg border border-gray-200 text-right text-[13px] focus:outline-none focus:ring-2 focus:ring-green-500/30"
                    />
                  </td>
                  <td className="px-6 py-4">
                    <input
                      ...
                      onChange={(e) => handleUpdate(limit.tier, "concurrentSessions", parseInt(e.target.value))}
                      className="w-32 px-3 py-1.5 rounded-lg border border-gray-200 text-right text-[13px] focus:outline-none focus:ring-2 focus:ring-green-500/30"
                    />
                  </td>
                </tr>
            ...
            return (
              <div key={limit.tier}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[12px] font-semibold text-green-900">{limit.tier.toUpperCase()}</span>
                  <span className="text-[11px] text-text-muted">{usagePercentage.toFixed(0)}% of limit</span>
                </div>
